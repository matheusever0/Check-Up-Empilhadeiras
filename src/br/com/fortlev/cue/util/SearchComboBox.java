 ...
 * @author josenito
 */
public class SearchComboBox extends PlainDocument {
    ...
    private Object lookupItem2(String pattern) {
        String buscar = pattern;
        Object selectedItem = model.getSelectedItem();
        if(selectedItem != null && startsWithIgnoreCase(selectedItem.toString(), pattern)) {
            buscar = selectedItem.toString();
        }
        ...
        for (int i = 0, n = model.getSize(); i < n; i++) {
            ...
            if (qualquerParte(currentItem.toString(), buscar)) {
              Object item = binaryLookup(currentItem.toString());
                return item;
            }
           }
          return null;
